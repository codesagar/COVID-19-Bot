...
DataFrame.fromCSV('https://coronacheck.z20.web.core.windows.net/manthan.csv').then(myQuestions);

function myAdvice(df) {
    advice_dict = df.toDict();
}

DataFrame.fromCSV('https://coronacheck.z20.web.core.windows.net/manthan_advice.csv').then(myAdvice);
...
var options_dict = {
	...
	"मराठी" : "option<br 1000> मी तुला काय मदत करू?<ar> कोरोना रिस्क स्कॅन <ar> सुरक्षितता मार्गदर्शक तत्त्वे",
	...
	$("#submit").click(function(event) {
		...
		if (get_contact) {
			...
			$.ajax({
				...
				// },
				data: {'name':name, 'contact':contact, 'message':message, 'referrer':document.referrer},
	...
	$('.chat-form').on("click", '.buttonResponse', function() {
		...
		if(get_contact){
			...
			// console.log("english_options".concat(eval(english_options[counter])))
 			...
 			console.log(multiSelect);
	        this.style.backgroundColor="rgb(64,172,246)";
...
function send(text, selected_id, selected_options) {
	...
	if (selectLang) {
 		...
 		userData['lang'] = languagePrefrence;
 		userData['answers'] = {};
 		userData['selections'] = {};
	...
	c	onsole.log('inside survey')
		if(get_contact == false){
			...
			function showPosition(position) {
				userData['accurate_location'] = {'lat':position.coords.latitude,'lon':position.coords.longitude};
			}
			...
			$.ajax({
				...
				newRecievedMessage("<br 2000>".concat(JSON.stringify(responseData.risk_score, undefined, 2)));
				newRecievedMessage(share_dict[languagePrefrence]);
				$('.share').delay(5000).show(0);
				$('.restart').delay(5000).show(0);	
